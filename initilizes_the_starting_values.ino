...
long testDumpLen = 24.5;
long testDumpWidth = 23.5;
long testDumpHeight =36;
//If there is a slant
long triangle_width = 0;
long triangle_height = 36;
//Calculated based off of casing
long topxoffset = 4.25;   // 4.25  
long bottomxoffset = 1.75; 
long topyoffset = 0.6;     
long bottomyoffset = 7.4;  // weird 7.4
...
//Commented these out to see if it works later
//const float EMPTY_READING_60 = 32.0;
//const float EMPTY_READING_15 = 19.0;

//These are to calculate the estimated dimensions
float ysubTop, ysubBottom, xsubTop, xsubBottom;
float DTop, DBottom;
...
void setup() {
  ...
  DTop = (testDumpHeight - topyoffset)*tan(radians(75));
  if (DTop >testDumpLen ) {
    xsubTop = (testDumpLen-topxoffset)/cos(radians(75));
    ysubTop = ((testDumpLen-topxoffset)/tan(radians(75)));
  } else {
    xsubTop = (testDumpHeight - topxoffset)/sin(radians(75));
    ysubTop = 0;
  }

  DBottom = (testDumpHeight - bottomyoffset)*tan(radians(30));
  if (DBottom >testDumpLen ) {
    xsubBottom = (testDumpLen-bottomxoffset)/cos(radians(30));
    ysubBottom = ((testDumpLen-bottomxoffset)/tan(radians(30)));
  } else {
    xsubBottom = (testDumpHeight - bottomxoffset)/sin(radians(30));
    ysubBottom = 0;
  }
  Serial.println(DTop);
...
float calculateVolume(float len, float width, float height,
  ...
  float h1_empty = ysubBottom + bottomyoffset;
  float x1_empty = xsubBottom + bottomxoffset;
  float h2_empty = ysubTop + topyoffset;
  float x2_empty = xsubTop + topxoffset;
  ...
  float currentEmptyVol = width * (
    ...
    - triangle_width * triangle_height *.5 // if there is a slant
  );
...
float processA02Data(unsigned char* buffer, int sensorNum) {
  if (buffer[0] == 0xFF) {
    unsigned char CS = (buffer[0] + buffer[1] + buffer[2]) & 0xFF;
    
    if (CS == buffer[3]) {
      int distance_mm = (buffer[1] << 8) | buffer[2];
      float distance_inches = distance_mm * 0.03937;
